...
Pyramiden- bzw. Kegelspitzen zu sehen sind, wie am Rand der Erdkugel in
...
k"onnte es zu "Uberschneidungen der Schneedecke mit sich selbst an solchen
Ecken kommen (siehe Bild 3). Da allerdings weder schwebende Kegelspitzen
...
der sich selbst schneidenden Displacement-Oberfl"ache kann durch die
Repr"asentation durch Voxel verwirklicht werden.\\
Dabei wird mit Hilfe von Voxeln eine
richtige Schneedecke erzeugt, die auf bzw. "uber der Szenenoberfl"ache
...
kann. Darauffolgend wird jeder Voxel gepr"uft, ob er innerhalb oder
...
sowie die L"osung des "Uberschneidungsproblems. W"achst zum Beispiel von
unten und von links der Schnee an, so trifft die Schneeoberfl"ache auf den
selben Voxel und
die Schneedecke erweitert sich bis zu diesem Punkt. Folglich kann es keine
ungewollten "Uberschneidungen geben. Die Bereiche ''Innen'' und ''Au\ss en''
stehen in der Zeichnung f"ur Gebiete innerhalb sowie au\ss erhalb von einem
Objekt.
\\
Im Folgenden wird erl"autert, was ein Voxel in diesem Fall genau ist, welche
...
Eigenschaften.
Ein Voxel besitzt folglich eine \texttt{x}-, \texttt{y}- und
\texttt{z}-Koordinate zur eindeutigen Festlegung der Position.
Des Weiteren wird gespeichert, ob ein Voxel sich inner- oder au\ss erhalb
eines Objekts befindet, was mit Hilfe des dreidimensionalen Punkt in Polygon
Algorithmus ermittelt wird. Zudem wird vorgehalten, ob ein Voxel Schnee
...
Ein erster, einfacherer Ansatz ist in Abbildung \ref{fig:voxel_grid_rect}
...
\end{figure}
Setzt man bei einer solchen beliebigen Oberfl"ache die Schnittpunkte allerdings
in die Kantenmitte, so erh"alt man eine initial angen"aherte L"osung, die durch
...
im Anschlu\ss\ die Nachbarschaftsbeziehungen f"ur jeden Voxel zu setzen.
Beispielhaft sei einmal die Bestimmung des rechten Nachbarvoxels aufgef"uhrt.
...
Nachbarvoxel auch rechts von dem aktuellen Voxel liegen sollte. Weiterhin
ist zu beachten, dass der Nachbarvoxel auf der selben H"ohe
(gleiche \texttt{z}-Koordinate) sowie direkt neben (gleiche
\texttt{y}-Koordinate) liegen muss.\\
...
kann je nach Granularit"at eine gewisse Zeit beanspruchen,
...
eine Kugel und im rechten die wiederhergestellte Oberfl"ache eines Kegels.
...
tats"achliche Objektoberfl"ache. Dies illustriert die Abbildung
...
\end{figure}
Allen drei gezeichneten Figuren liegt das selbe Objekt zugrunde.
Das erste Bild hat offensichtlich eine sehr geringe Voxeldichte oder auch
Granularit"at von 5. Das hei\ss t pro Einheit auf der jeweiligen Achse
...
angen"ahert werden kann, ist es im Folgenden m"oglich Schnee auf Objekten
...
Abbildung \ref{fig:rect_obj} zu sehen ist, sehr zufriedenstellend.
\begin{figure}[htbp]
    \centering
...
\end{figure}
Die Abbildung \ref{fig:voxel_grid_snow} stellt das das Voxelgitter schematisch
...
schwarzen Kreise stellen den jeweiligen \texttt{density}- oder F"ullewert
...
den jeweiligen Voxel trifft. "Uberschreitet der F"ullwert dabei den Wert
...
Kandidaten in dem Array \texttt{activeVoxels} zu speichern.
...
\texttt{if}-Statement (Zeile 6 - 11) fest, welche Voxel als m"ogliche
Landepl"atze bereitgestellt werden. Dies betrifft zum einen alle auf der
Schneeoberfl"ache liegenden Voxel, das bedeutet, Voxel die selbst Schnee
enthalten, ihr oberer Nachbar aber nicht. Zum anderen soll auch neben den
Objekten, auf dem Szenenboden Schnee fallen k"onnen, wozu die Voxel ohne
...
folgendem Ergebnis (Abbildung \ref{fig:topSnow}). Der Pfeil zeigt die Richtung
des Schneefalls an.
\begin{figure}[htbp]
    \centering
...
vorkommen, dass auch Voxel unterhalb eines Objekts auf den Boden empf"anglich
...
Um nun etwas Wind zu simulieren wird zus"atzlich zu dem senkrechten noch
waagerechter Schneefall hinzugef"ugt. Die Funktionsweise ist analog zum
Schnee von oben, nur das etwas andere Voxel aktiviert werden.
...
Ist dies der Fall, so kann die Schneedecke nun rechts an das Objekt wachsen.
...
deutlich noch oben und nach rechts gewachsene Schneedecke, da sich die
...
''Computer Modelling Of Fallen Snow'' von Paul Fearing in dem
