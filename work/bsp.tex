...
\begin{wrapfigure}{r}{0.4\textwidth}
    ...
    \end{center}
    \vspace{-10pt}
    \caption{Beispiel f"ur Displacement Mapping}
...
In der Abbildung \ref{fig:displacement}\cite{displacement_mapping_pic} ist ein
...
Da sich bei dieser Vertexverschiebung auch die Normalen "andern
...
Stelle der Szene zu speichern (Snow-Map).
Diese Textur k"onnte anschlie\ss end als Lightmap
interpretiert werden, das hei\ss t, dass die urspr"ungliche Farbe der
Objekttextur entsprechend der Menge an Schnee aufgehellt wird. Das Prinzip
einer Lightmap ist in der folgenden Abbildung (\ref{fig:lightmap}) verdeutlicht.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=300px]{pictures/lightmap.png}
    \caption{Lightmap eines W"urfels}
    \label{fig:lightmap}
\end{figure}

Dabei hat der W"urfel lediglich eine Textur, die eine nicht vorhandene
Lichtquelle simuliert. Ebenso w"urde sich die Snow-Map mit der Simulation von
Schnee verhalten. Die dabei
...
mit gewissen Problemen konfrontiert. Zum einen wird keine richtige
Schneedecke erzeugt, sondern lediglich durch Modifizierung der Objektgeometrie
simuliert und zum anderen k"onnen unerw"unschte Nebeneffekte auftreten.
...
der sich ver"andernden Objektgeometrie und eventuell wachsenden Schneemengen
k"onnte es zu "Uberschneidungen des Objektes mit sich selbst an solchen Ecken
kommen (siehe Bild 3). Da allerdings weder die Objektgeometrie ver"andert, noch
seltsame Effekte beim Rendern durch "Uberschneidungen auftreten sollen,
...
was in der Zeichnung angedeutet wird. Auch die Seite $\overrightarrow{P_2 P_1}$
selbst kann als
...
Edge-Array gespeichert, wobei eine Kante immer aus vier Koordinaten besteht,
